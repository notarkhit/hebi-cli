...
from hebi.utils import hypr
from hebi.utils.notify import notify
from hebi.utils.paths import c_config_dir, screenshots_cache_dir, screenshots_dir
...
class Command:
    ...
    def region(self) -> None:
        if self.args.region == "slurp":
            subprocess.run(
                ["qs", "-p", str(c_config_dir), "ipc", "call", "picker", "openFreeze" if self.args.freeze else "open"]
            )
        else:
